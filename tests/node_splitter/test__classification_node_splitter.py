...
from smarttree._node_splitter import ClassificationNodeSplitter
from smarttree._tree_node import TreeNode


@pytest.fixture(scope="module")
def classification_node_splitter(
    X, y, numerical_feature_names, categorical_feature_names, rank_feature_names
) -> ClassificationNodeSplitter:
    return ClassificationNodeSplitter(
        X=X,
        y=y,
        criterion="gini",
        max_depth=None,
...
def test__is_splittable(classification_node_splitter, X):
    node = TreeNode(
        number=0,
        samples=X.shape[0],
        depth=0,
    )
    classification_node_splitter.is_splittable(node)


def test__find_best_split(classification_node_splitter, X, y):
    parent_mask = y.apply(lambda x: True)
    available_feature_names = X.columns.tolist()
    leaf_counter = 0
    classification_node_splitter.find_best_split(
