...
def test__gini_index(dataset):
    ...
    class_names = np.sort(dataset.y.unique())
    y = dataset.y.values

    gini_index = cy_base_column_splitter.gini_index(mask, y, class_names)
...
def test__entropy(dataset):
    ...
    gini_index = cy_base_column_splitter.entropy(mask, y, class_names)
